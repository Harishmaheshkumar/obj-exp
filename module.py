...
def foldername(name):
    ...
    while(cap.isOpened()):
        ...
        try:
            ...
            data="UP: "+str(cnt_up)+"\tDOWN: "+str(cnt_down)+"\tTOTAL CARS: "+str(total)
            #data="Total cars:  "+str(total)
            df={'Up':[cnt_up],'DOWN':[cnt_down],'Total cars':[total]}
            df_data=pd.DataFrame(df)
            df_data.to_csv("csv\\report.csv",index=False)
            break
